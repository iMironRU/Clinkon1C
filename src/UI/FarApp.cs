...
public class FarApp
{
    ...
    private void Rescan()
    {
        ...
        // Параллельный скан (особенно измерение кэша — сотни тысяч файлов,
        // сотни ГБ) даёт большой всплеск временных аллокаций. Без явного
        // Collect() рантайм не спешит возвращать память ОС — процесс выглядит
        // "прожорливым" в диспетчере задач ещё долго после того, как скан
        // закончился и данные больше не нужны.
        GC.Collect();
...
@"  ↑ ↓ PgUp PgDn Home End   Навигация
  ...
  F10                        Выход");
    ...
    private void DoTjAnalyze()
    {
        ...
        // events может держать до 100 000 TjEvent с полным Descr/Context
        // (до ~5000 симв. каждый — сотни МБ на большом наборе логов). Пока
        // пользователь листал анализ, эти объекты могли дожить до Gen2, где
        // сборка происходит редко — после закрытия экрана прибираем явно,
        // а не ждём, пока рантайму самому "захочется".
        GC.Collect();
    }
